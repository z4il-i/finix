  ...
  lib.extendMkDerivation {
    ...
    extendDrvArgs =
      ...
      let
        ...
        buildFHSEnv = callPackage ./buildFHSEnv.nix { inherit config; };
          ...
          ''
            ...
            for i in ${fhsenv}/*; do
              ...
              if [[ $path == '/etc' ]]; then
                ...
                symlinks+=(--symlink "$(${lib.getExe' config.programs.coreutils.package "readlink"} "$i")" "$path")
                ignored+=("$path")
              else
        ...
        bin = writeScript "${name}-bwrap" (bwrapCmd {
          initArgs = ''"$@"'';
        });
      in
      {
